...
CONTAINS
   ...
   ! Perform the operation target = target + alpha*source^T
   SUBROUTINE add_scaled_transpose_matrix(alpha, source, TARGET)
      REAL*8, INTENT(IN)        :: alpha
      REAL*8, INTENT(IN)        :: source(:, :)
      REAL*8, INTENT(INOUT)     :: TARGET(:, :)
      INTEGER*4                 :: j, nrow, ncol

      nrow = SIZE(source, 1)
      ncol = SIZE(source, 2)

      DO j = 1, nrow
         CALL DAXPY(ncol, alpha, source(j, 1), nrow, TARGET(1, j), 1)
      END DO
   END SUBROUTINE add_scaled_transpose_matrix

   ! Perform the operation target = target + alpha*source
   SUBROUTINE add_scaled_matrix(alpha, source, TARGET)
      REAL*8, INTENT(IN)        :: alpha
      REAL*8, INTENT(IN)        :: source(:, :)
      REAL*8, INTENT(INOUT)     :: TARGET(:, :)
      INTEGER*4                 :: nentries

      nentries = SIZE(source, 1)*SIZE(source, 2)
      CALL DAXPY(nentries, alpha, source(1, 1), 1, TARGET(1, 1), 1)
   END SUBROUTINE add_scaled_matrix
   ...
   ! Tensor product
   !*******************************************
   PURE FUNCTION TensorProduct(Vl, Vr) RESULT(Tens)
      REAL, INTENT(IN), DIMENSION(:)      :: Vl, Vr
      REAL, DIMENSION(SIZE(Vl), SIZE(Vr)) :: Tens

      INTEGER                              :: j, Nr

      Nr = SIZE(Vr)
      DO CONCURRENT(j=1:Nr)
         Tens(:, j) = Vl*Vr(j)
      END DO
   ...
   !*******************************************
   PURE SUBROUTINE TensorProductCumul(M, Vl, Vr)
      REAL, INTENT(INOUT), DIMENSION(:, :)     :: M
      REAL, INTENT(IN), DIMENSION(:)        :: Vl, Vr
   ...
   !*******************************************
   PURE FUNCTION TensorSumInt(Vl, Vr) RESULT(Tens)
      INTEGER, INTENT(IN), DIMENSION(:)      :: Vl, Vr
   ...
   !****************************************************
   PURE FUNCTION colint(M) RESULT(V)
