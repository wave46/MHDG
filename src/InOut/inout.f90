...
CONTAINS
   ...
   SUBROUTINE HDF5_save_solution(fname)
...
#else
      USE communications, ONLY: gather_mesh, gather_solution_3D, gather_additional, gather_magnetic_field
#endif
#endif
      IMPLICIT NONE
      ...
      IF ((switch%shockcp .NE. 0) .OR. ((adapt%shockcp_adapt .NE. 0) .AND. (adapt%adaptivity .EQV. .TRUE.))) THEN
      ...
      ! gather solution, mesh and magnetic related arrays
      CALL gather_solution_3D(Mesh, sol%u, sol%u_tilde, sol%q, u_glob, u_tilde_glob, q_glob)
   ...
   SUBROUTINE HDF5_load_mesh_from_solution(fname)
      ...
      IF (utils%printint > 0) THEN
         IF (MPIvar%glob_id .EQ. 0) THEN
            WRITE (*, *) "*************************************************"
            WRITE (*, *) "            LOADING MESH FROM SOLUTION           "
            WRITE (*, *) "*************************************************"
         END IF
      END IF
   ...
   END SUBROUTINE HDF5_load_mesh_from_solution
   ...
   SUBROUTINE HDF5_load_solution(fname)
      ...
      IF (INDEX(fname, "Sol3D") .NE. 0) THEN
      ...
      ELSEIF (INDEX(fname, "Sol2D") .NE. 0) THEN
         ! Initialization with a 2D solution

         IF (MPIvar%glob_id .EQ. 0) THEN
            WRITE (6, *) '**************************************************'
            WRITE (6, *) '* 2D INITIAL SOLUTION: PROPAGATING in THE THORUS *'
            WRITE (6, *) '**************************************************'
         END IF
         ...
         CALL HDF5_open(fname_complete, file_id, IERR)
         CALL HDF5_group_open(file_id, 'solution', group_id, ierr)
         CALL HDF5_array1D_reading(group_id, u_aux, 'u')
         CALL HDF5_array1D_reading(group_id, u_tilde_aux, 'u_tilde')
         CALL HDF5_array1D_reading(group_id, q_aux, 'q')
         CALL HDF5_group_close(group_id, ierr)
         CALL HDF5_close(file_id)
...
#endif
      IF (switch%ME) THEN
         CALL HDF5_group_open(group_id, 'time', group_id, ierr)
         CALL HDF5_integer_reading(group_id, it, 'Current_time_step_number')
         CALL HDF5_real_reading(group_id, t, 'Current_time')
         IF (it .GT. 1) THEN
            time%it = it
            time%ik = it
            time%t = t
   ...
   SUBROUTINE copy_file(source_file, destination_file)
      ...
      DO
         ...
         WRITE (unit_out, '(a)') TRIM(ADJUSTL(buffer))
      END DO
