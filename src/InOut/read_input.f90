...
SUBROUTINE READ_input()
  ...
  LOGICAL               :: neutral_wall_sources_in_elements, neutral_recycling_in_elements
  ...
  NAMELIST /SWITCH_LST/ steady,read_gmsh, readMeshFromSol, set_2d_order, order_2d, gmsh2h5, axisym,external_heating, impurity_radiation, init, driftdia, driftexb, testcase, OhmicSrc, ME,diff_reverse_Ip, target_variable, RMP, Ripple, psdtime, diffred, diffmin, &
       ...
       & logrho,bxgradb,flux_limiter,import_diffusion_1D,neutral_wall_sources_in_elements,neutral_recycling_in_elements,neutral_perpendicular_diffusion
  ...
  neutral_recycling_in_elements = .false.
  ...
  CASE ('off', 'diagnostics_only', 'lagged_flux_limiter')
  CASE DEFAULT
     PRINT *, 'Unknown neutral_flux_limiter_mode: ', TRIM(ADJUSTL(neutral_flux_limiter_mode))
  ...
  switch%neutral_recycling_in_elements = neutral_recycling_in_elements
  ...
  phys%recycling_neutral_gamma = recycling_neutral_gamma
  ...
  IF (MPIvar%glob_id .EQ. 0) THEN
     ...
     PRINT *, '                - neutral recycling in element volumes:              ', switch%neutral_recycling_in_elements
