...
SUBROUTINE HDG_computeJacobian()
  ...
  IF (switch%neutral_recycling_in_elements) THEN
    CALL account_neutral_recycling_element_totals()
  ENDIF
#endif
#endif
...
CONTAINS
  ...
  SUBROUTINE account_neutral_recycling_element_totals()

    INTEGER :: ifac,el,fa,fl,bc,g,idm
    INTEGER :: ind_nodes(refElPol%Nfacenodes)
    REAL*8  :: Xf(refElPol%Nfacenodes,2),Bf(refElPol%Nfacenodes,3),psif(refElPol%Nfacenodes)
    REAL*8  :: uef(refElPol%Nfacenodes,phys%Neq),qef(refElPol%Nfacenodes,phys%Neq*Ndim)
    REAL*8  :: q_cylf(refElPol%Nfacenodes)
    REAL*8  :: xyg(refElPol%NGauss1D,2),xyder(refElPol%NGauss1D,2),psig(refElPol%NGauss1D)
    REAL*8  :: ueg(refElPol%NGauss1D,phys%Neq),qeg(refElPol%NGauss1D,phys%Neq*Ndim)
    REAL*8  :: upg(refElPol%NGauss1D,phys%npv)
    REAL*8  :: Bmod_nod(refElPol%Nfacenodes),b_nod(refElPol%Nfacenodes,3),b(refElPol%NGauss1D,3)
    REAL*8  :: q_cyl(refElPol%NGauss1D),rho_pol_norm(refElPol%NGauss1D)
    REAL*8  :: diff_iso(refElPol%NGauss1D),diff_ani(refElPol%NGauss1D)
    REAL*8  :: diff_iso_fac(phys%Neq,phys%Neq,refElPol%NGauss1D)
    REAL*8  :: diff_ani_fac(phys%Neq,phys%Neq,refElPol%NGauss1D)
    REAL*8  :: APinch(phys%Neq,Ndim),Qpr(Ndim,phys%Neq)
    REAL*8  :: dline,xyDerNorm_g,t_g(Ndim),ng(Ndim),bn,pinch_dot
    REAL*8  :: recycling_coeff,RN,E,theta,recycling_source,total_recycling,dim_factor

    total_recycling = 0.d0
    dim_factor = 2.d0*PI*simpar%refval_density*simpar%refval_speed*simpar%refval_length**2
    ...
    DO ifac = 1,Mesh%Nextfaces
      ...
      bc = phys%bcflags(fl)
      IF ((bc .NE. bc_Bohm) .AND. (bc .NE. bc_BohmPuff) .AND. (bc .NE. bc_BohmPump)) CYCLE

      el = Mesh%extfaces(ifac,1)
      fa = Mesh%extfaces(ifac,2)
      ind_nodes = (el - 1)*Npel + refElPol%face_nodes(fa,:)
      Xf = Mesh%X(Mesh%T(el,refElPol%face_nodes(fa,:)),:)
      Bf = phys%B(Mesh%T(el,refElPol%face_nodes(fa,:)),:)
      psif = phys%magnetic_psi(Mesh%T(el,refElPol%face_nodes(fa,:)))
      uef = ures(ind_nodes,:)
      qef = qres(ind_nodes,:)
      IF (switch%testcase .EQ. 60) THEN
        q_cylf = geom%q
      ELSE
        q_cylf = phys%q_cyl(Mesh%T(el,refElPol%face_nodes(fa,:)))
      ENDIF

      xyg = MATMUL(refElPol%N1D,Xf)
      xyder = MATMUL(refElPol%Nxi1D,Xf)
      ueg = MATMUL(refElPol%N1D,uef)
      qeg = MATMUL(refElPol%N1D,qef)
      psig = MATMUL(refElPol%N1D,psif)
      q_cyl = MATMUL(refElPol%N1D,q_cylf)

      Bmod_nod = SQRT(Bf(:,1)**2 + Bf(:,2)**2 + Bf(:,3)**2)
      b_nod(:,1) = Bf(:,1)/Bmod_nod
      b_nod(:,2) = Bf(:,2)/Bmod_nod
      b_nod(:,3) = Bf(:,3)/Bmod_nod
      b = MATMUL(refElPol%N1D,b_nod)
      rho_pol_norm = SQRT(MAX(psig,1.e-10))

#ifndef KEQUATION
      CALL setLocalDiff(xyg,ueg,diff_iso_fac,diff_ani_fac)
#else
      CALL setLocalDiff(xyg,ueg,diff_iso_fac,diff_ani_fac,q_cyl)
#endif
      ...
      ENDIF
      diff_iso = diff_iso_fac(1,1,:)
      diff_ani = diff_ani_fac(1,1,:)
      CALL cons2phys(ueg,upg)
      ...
      DO g = 1,refElPol%NGauss1D
        ...
        ng = [t_g(2),-t_g(1)]
        bn = DOT_PRODUCT(b(g,1:2),ng)

        RN = 1.d0
        IF (phys%apply_trim) THEN
          E = 0.5d0/simpar%refval_charge*simpar%refval_mass*(simpar%refval_speed*upg(g,2))**2
          theta = 180.d0/ACOS(-1.d0)*ACOS(MIN(1.d0,ABS(bn)))
          CALL compute_RN(E,theta,RN)
        ENDIF

        APinch = 0.d0
        IF (switch%transport_1d) THEN
          CALL transport_model_1d%compute_1D_pinch_matrix(b(g,:),SQRT(MAX(psig(g),0.d0)),APinch)
        ENDIF
        pinch_dot = APinch(1,1)*ng(1) + APinch(1,2)*ng(2)
        Qpr = RESHAPE(qeg(g,:),(/Ndim,phys%Neq/))

        recycling_source = recycling_coeff*RN*(ueg(g,2)*bn + ueg(g,1)*pinch_dot)
        DO idm = 1,Ndim
          recycling_source = recycling_source &
            &- recycling_coeff*RN*(ng(idm)*diff_iso(g) - bn*b(g,idm)*diff_ani(g))*Qpr(idm,1)
        ENDDO
        total_recycling = total_recycling + recycling_source*dline*dim_factor
      ENDDO
    ENDDO

    CALL diag%add(diag_category_particles, diag_particle_recycling_source, total_recycling)
#ifdef PARALL
    CALL MPI_ALLREDUCE(MPI_IN_PLACE, total_recycling, 1, MPI_REAL8, MPI_SUM, MPI_COMM_WORLD, ierr)
#endif
  ENDSUBROUTINE account_neutral_recycling_element_totals
#endif
#endif
...
!#endif
    ...
    ENDIF
    IF (switch%neutral_recycling_in_elements) THEN
      CALL add_neutral_recycling_to_element(iel,Xel,Bel,psiel,q_cylel,ue,Auu,Auq,rhs)
    ENDIF
#endif
      CALL do_assembly(Auq,Auu,rhs,ind_ass,ind_asq,iel)
      DEALLOCATE(Auq,Auu,rhs)
...
#ifdef NEUTRAL
  SUBROUTINE compute_element_volume_moments(Xel,element_volume,VolNi,VolNNi)

    REAL*8,INTENT(IN)                  :: Xel(:,:)
    REAL*8,INTENT(OUT)                 :: element_volume
    REAL*8,INTENT(OUT)                 :: VolNi(:)
    REAL*8,INTENT(OUT),OPTIONAL        :: VolNNi(:,:)
    INTEGER                            :: g
    ...
    element_volume = 0.d0
    VolNi = 0.d0
    IF (PRESENT(VolNNi)) VolNNi = 0.d0

    DO g = 1,refElPol%Ngauss2d
      dvolu = refElPol%gauss_weights2D(g)*detJ(g)
      IF (switch%axisym) dvolu = dvolu*xyv(g,1)
      element_volume = element_volume + dvolu
      VolNi = VolNi + refElPol%N2D(g,:)*dvolu
      IF (PRESENT(VolNNi)) THEN
        VolNNi = VolNNi + tensorProduct(refElPol%N2D(g,:)*dvolu,refElPol%N2D(g,:))
      ENDIF
    ENDDO

    IF (element_volume <= 0.d0) THEN
      WRITE(6,*) 'Negative or zero element volume while spreading neutral wall sources'
      STOP
    ENDIF

    VolNi = VolNi/element_volume
    IF (PRESENT(VolNNi)) VolNNi = VolNNi/element_volume

  ENDSUBROUTINE compute_element_volume_moments
  ...
  SUBROUTINE add_neutral_wall_sources_to_element(iel,Xel,ue,Auu,rhs)
    ...
    IF ((puff_integral .NE. 0.d0) .OR. (pump_integral .NE. 0.d0)) THEN
      CALL compute_element_volume_moments(Xel,element_volume,VolNi,VolNNi)

      puff_source = puff_integral/element_volume
      pump_source = pump_integral/element_volume

      rhs(:,inn) = rhs(:,inn) + puff_integral*VolNi
      Auu(:,:,z) = Auu(:,:,z) + pump_integral*VolNNi
  ...
  SUBROUTINE add_neutral_recycling_to_element(iel,Xel,Bel,psiel,q_cylel,ue,Auu,Auq,rhs)

    INTEGER,INTENT(IN)       :: iel
    REAL*8,INTENT(IN)        :: Xel(:,:),Bel(:,:),psiel(:),q_cylel(:),ue(:,:)
    REAL*8,INTENT(INOUT)     :: Auu(:,:,:),Auq(:,:,:),rhs(:,:)
    INTEGER                  :: ifa,iface,ibf,fl,bc
    INTEGER                  :: g,idm,inn,z_parallel,z_pinch,zq
    INTEGER,PARAMETER        :: plasma_density_eq = 1, plasma_parallel_eq = 2
    REAL*8                   :: Xfl(refElPol%Nfacenodes,2),Bfl(refElPol%Nfacenodes,3)
    REAL*8                   :: uef(refElPol%Nfacenodes,phys%Neq)
    REAL*8                   :: xyg(refElPol%Ngauss1d,2),xyder(refElPol%Ngauss1d,2)
    REAL*8                   :: ueg(refElPol%Ngauss1d,phys%Neq)
    REAL*8                   :: upg(refElPol%Ngauss1d,phys%npv)
    REAL*8                   :: psig(refElPol%Ngauss1d),q_cyl(refElPol%Ngauss1d),rho_pol_norm(refElPol%Ngauss1d)
    REAL*8                   :: Bmod_nod(refElPol%Nfacenodes),b_nod(refElPol%Nfacenodes,3),bvec(refElPol%Ngauss1d,3)
    REAL*8                   :: diff_iso_fac(phys%Neq,phys%Neq,refElPol%Ngauss1d)
    REAL*8                   :: diff_ani_fac(phys%Neq,phys%Neq,refElPol%Ngauss1d)
    REAL*8                   :: dline,element_volume
    REAL*8                   :: xyDerNorm_g,t_g(Ndim),ng(Ndim),bn,pinch_dot
    REAL*8                   :: recycling_coeff,RN,E,theta
    REAL*8                   :: APinch(phys%Neq,Ndim)
    REAL*8                   :: parallel_integral,pinch_integral,diffusion_integral(Ndim)
    REAL*8,DIMENSION(Npel)   :: VolNi
    REAL*8,DIMENSION(Npel,Npel) :: VolNNi

    inn = phys%idx_rhon_eq
    IF (inn <= 0) RETURN
    z_parallel = inn + (plasma_parallel_eq - 1)*Neq
    z_pinch = inn + (plasma_density_eq - 1)*Neq
    ...
    CALL compute_element_volume_moments(Xel,element_volume,VolNi,VolNNi)
    parallel_integral = 0.d0
    pinch_integral = 0.d0
    diffusion_integral = 0.d0
    ...
    DO ifa = 1,refElPol%Nfaces
      ...
      Xfl = Xel(refElPol%face_nodes(ifa,:),:)
      Bfl = Bel(refElPol%face_nodes(ifa,:),:)
      uef = ue(refElPol%face_nodes(ifa,:),:)

      xyg = MATMUL(refElPol%N1D,Xfl)
      xyder = MATMUL(refElPol%Nxi1D,Xfl)
      ueg = MATMUL(refElPol%N1D,uef)
      psig = MATMUL(refElPol%N1D,psiel(refElPol%face_nodes(ifa,:)))
      q_cyl = MATMUL(refElPol%N1D,q_cylel(refElPol%face_nodes(ifa,:)))

      Bmod_nod = SQRT(Bfl(:,1)**2 + Bfl(:,2)**2 + Bfl(:,3)**2)
      b_nod(:,1) = Bfl(:,1)/Bmod_nod
      b_nod(:,2) = Bfl(:,2)/Bmod_nod
      b_nod(:,3) = Bfl(:,3)/Bmod_nod
      bvec = MATMUL(refElPol%N1D,b_nod)
      rho_pol_norm = SQRT(MAX(psig,1.e-10))
      ...
      DO g = 1,refElPol%Ngauss1d
        ...
        APinch = 0.d0
        IF (switch%transport_1d) THEN
          CALL transport_model_1d%compute_1D_pinch_matrix(bvec(g,:),SQRT(MAX(psig(g),0.d0)),APinch)
        ENDIF
        pinch_dot = APinch(1,1)*ng(1) + APinch(1,2)*ng(2)

        parallel_integral = parallel_integral + recycling_coeff*RN*bn*dline
        pinch_integral = pinch_integral + recycling_coeff*RN*pinch_dot*dline
        DO idm = 1,Ndim
          diffusion_integral(idm) = diffusion_integral(idm) &
            &- recycling_coeff*RN*(ng(idm)*diff_iso_fac(1,1,g) &
            &- bn*bvec(g,idm)*diff_ani_fac(1,1,g))*dline
        ENDDO
      ENDDO
    ENDDO

    Auu(:,:,z_parallel) = Auu(:,:,z_parallel) + parallel_integral*VolNNi
    Auu(:,:,z_pinch) = Auu(:,:,z_pinch) + pinch_integral*VolNNi
    DO idm = 1,Ndim
      zq = inn + (idm - 1)*Neq + (plasma_density_eq - 1)*Ndim*Neq
      Auq(:,:,zq) = Auq(:,:,zq) + diffusion_integral(idm)*VolNNi
    ENDDO
