...
SUBROUTINE solve_global_system()
   ...
   USE globals
   USE LinearAlgebra
   USE MPI_OMP!, only: MPIvar!, MPI_COMM_WORLD, MPI_IN_PLACE,MPI_INT,MPI_REAL8,MPI_MAX,MPI_SUM
#ifdef WITH_PASTIX
...
#endif
#ifdef WITH_PETSC
   USE solve_petsc, ONLY: matPETSC, init_mat_PETSC, preallocate_matrix_PETSC, fill_rhs_PETSC, build_mat_PETSC, setup_ksp_pc_settings, solve_mat_PETSC, petsc_retrieve_array, petsc_set_array
#endif
#ifdef PARALL
   USE Communications, ONLY: exchangeSol
#endif
   ...
   ELSE IF (lssolver%sollib .EQ. 3) THEN
      ...
      IF (matK%start) THEN
         ...
         CALL fill_rhs_PETSC(matPETSC)
         CALL build_mat_PETSC(matPETSC)
         CALL setup_ksp_pc_settings(matPETSC, ir)
         ...
         matK%start = .FALSE.
      ELSE
         ! init mat petsc is called even for non-starting iterations because adaptivity can change the mesh
         CALL init_mat_PETSC(matPETSC)
         CALL fill_rhs_PETSC(matPETSC)
         CALL build_mat_PETSC(matPETSC)
         CALL extractInitialGuess_PETSC()
...
#ifdef PARALL
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, errsol, 1, MPI_REAL8, MPI_MAX, MPI_COMM_WORLD, ierr)
#endif
...
CONTAINS
   ...
   SUBROUTINE storeFaceSolution
      ...
      END IF
#endif
   END SUBROUTINE storeFaceSolution
   ...
   END SUBROUTINE extractInitialGuess_PETSC
#endif
