...
#else
   USE LinearAlgebra, ONLY: tensorSumInt, tensorProduct, invert_matrix, colint, col
#endif
...
CONTAINS
   ...
   SUBROUTINE assemblyVolumeContribution(NxNy_ax, NN, Aqq, Aqu)
      USE LinearAlgebra, ONLY: add_scaled_matrix
      REAL*8, INTENT(IN)        :: NN(:, :), NxNy_ax(:, :, :)
      REAL*8, INTENT(INOUT)     :: Aqq(:, :), Aqu(:, :, :)
      INTEGER*4                 :: k

      ! Aqq = Aqq + NN
      CALL add_scaled_matrix(1.d0, NN, Aqq)
      DO k = 1, Ndim
         ! Aqu(:, :, k) = Aqu(:, :, k) + NxNy_ax(:, :, k)
         CALL add_scaled_matrix(1.d0, NxNy_ax(:, :, k), Aqu(:, :, k))
      END DO
