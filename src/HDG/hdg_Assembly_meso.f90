...
!!$OMP END PARALLEL
  ...
  DO itor = 1, Ntorloc
     ...
     IF (itorg .EQ. numer%ntor + 1) itorg = 1
#else
     itorg = itor
#endif
     !**************************
     ...
     DO iel = 1, Nel
        ...
        DO ifa = 1, Nf + 2
           ...
           IF (ifa .EQ. 1) THEN
              ...
              IF ((MPIvar%ntor .GT. 1) .AND. (itor .EQ. 1)) CYCLE ! toroidal ghost face
#endif
              ...
              IF (MPIvar%itor .EQ. MPIvar%ntor .AND. itor .EQ. ntorass + 1) Fig = iel
#endif

              blk = blkp                        ! Number of nodes of the face
              CALL getindpospol(indpos)

           ELSEIF (ifa .EQ. Nf + 2) THEN
              !******************************************************
              ! ********* Second poloidal face of the element *******
              !******************************************************
#ifdef PARALL
              IF (Mesh%ghostElems(iel) .EQ. 1) CYCLE
              IF (itor .EQ. ntorass + 1) CYCLE
#endif
              ...
              IF (MPIvar%ntor .GT. 1 .AND. MPIvar%itor .EQ. MPIvar%ntor .AND. itor .EQ. ntorass) THEN
                 ! Toroidal parallelization
                 Fig = iel
              ELSE IF (MPIvar%ntor .EQ. 1 .AND. itor .EQ. ntorass) THEN
           ...
           ELSE
              ...
              IF (Fi .LE. Nintf) THEN
                 ...
                 ! Interior face
                 !**************************
                 CALL getindposint(indpos, Neq, Nfp, Nf, iel, Fi)
              ...
              ELSE
                 ...
                 CALL getindposext(indpos, Fe, Fd, Nf, Neq, Nfp)
...
CONTAINS
  ...
  SUBROUTINE computennz()
    ...
    DO itor = 1, Ntorass
       ...
       DO iel = 1, Nel
#ifdef PARALL
          IF ((MPIvar%ntor .GT. 1) .AND. (itor .EQ. 1)) CYCLE
#endif
          ! Increment face count
          Fig = Fig + 1
#ifdef PARALL
          ...
          IF (Ntorloc .GT. 1) THEN                  ! if only 1 toroidal division, nothing to add
             IF (Ntorloc .EQ. 2) THEN
       ...
       DO Fi = 1, Nfaces
          ...
          IF (Fi .LE. Nintf) THEN
             ...
             DO ieloc = 1, 2
                ...
                DO ifl = 1, Nf
                   IF (Fd(ifl)) CYCLE
                   IF (Fe(ifl) .EQ. Fi .AND. ieloc .EQ. 2) CYCLE
                   nnz = nnz + blkt**2
                   linew(Fig) = linew(Fig) + blkt
                END DO
             END DO
  ...
  SUBROUTINE getindpospol(indpos)
    ...
    IF (MPIvar%ntor .GT. 1) THEN
       ...
       ELSE
          ...
          ELSE IF (itorg .EQ. numer%ntor) THEN
             ...
             pos_start(2:Nf + 2) = pos_start(2:Nf + 2) + blkp
             pos_start(Nf + 2) = 0
          ELSE IF ((ifa .EQ. Nf + 2) .AND. itorg .EQ. numer%ntor-1) THEN
    ...
    ELSE
       ...
       ELSE
          ...
          IF (itor .EQ. 1) THEN
             ...
             pos_start(2:Nf + 2) = pos_start(2:Nf + 2) + blkp
             pos_start(Nf + 2) = pos_start(Nf + 2) + qq*blkt
          ELSE IF (itor .EQ. Ntorloc .AND. Ntorloc .EQ. 2) THEN
             ! case last set with only 2 toroidal divisions
             ! GLOB. EQ.    0 x x x 0     x x x
             !             [O x x x O]  O x x x O
             pos_start(1) = blkp + qq*blkt
          ...
          ELSE IF (itor .EQ. Ntorloc) THEN
             ...
             pos_start(2:Nf + 2) = pos_start(2:Nf + 2) + blkp
             pos_start(Nf + 2) = 0
          ELSE IF ((ifa .EQ. Nf + 2) .AND. itor .EQ. Ntorloc-1) THEN
             ! case second-last set
             ! GLOB. EQ.    O O x x x 0    x x x
             !                O x x x O [O x x x O]
             pos_start = pos_start + blkp
  ...
  !**************************************************
  SUBROUTINE getindposint(indpos, Neq, Nfp, Nf, iel)
    ...
    DO i = 1, Nf
       ...
       END DO
       DO j = 1, Nf
          IF (Fen(j) .EQ. Fi) CYCLE
          IF (Fdn(j)) CYCLE
          IF (Fe(i) .GT. Fen(j)) THEN
             pos(i) = pos(i) + 1
          ENDIF
       END DO
    END DO
    ...
    IF (itor .EQ. numer%ntor .AND. numer%ntor .GT. 1) THEN
       pos_start = pos_start + 2*blkp
       pos_start(Nf + 2) = 0
       IF (iel .GT. ieln) THEN
          pos_start(Nf + 2) = pos_start(Nf + 2) + blkp
       END IF
    ELSE IF (numer%ntor .EQ. 1) THEN
       IF (iel .GT. ieln) THEN
          pos_start(Nf + 2) = blkp
       ELSE
          pos_start(Nf + 2) = 0
...
SUBROUTINE HDG_assembly()
  ...
  INTEGER, POINTER                :: cols(:), rows(:), loc2glob(:)
  ...
  n_periodic_faces = COUNT(Mesh%periodic_faces .NE. 0)
  ...
  !**************************
  ! Parallel Loop over Mesh Elements
  !**************************

  !!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(iel, Fe, Fd, Fi, ifa, indpos, indglo, ieln, ifan, Fi_per, Fe_aux, Kel, fel)
  ...
  DO iel = 1, Nel
     ...
     CALL computeElementalMatrix(Kel, fel, iel)
     ...
     DO ifa = 1, Nf
        ...
        IF (Fi .LE. Nintf) THEN
           ...
           ! Interior face
           !**************************
           CALL getindposint(indpos, Fd, Fe, Neq, Nfp, Nf, iel, Fi)
        ...
        ELSE
           ...
           ELSE
              ...
              CALL getindposextperiodic(indpos, Fi_per, Fi, Nf, Nfp, neq, Fd, Fe, ieln)

              !!$OMP CRITICAL
#ifdef PARALL
              CALL fill_cols_vals_rowptr_loc2glob(Neq, Nfp, Nf, Fi, blk, ifa, Fe, Fd, indpos, indglo, Kel, fel, rows, rhsvec, loc2glob, cols, vals, shift, linew, Fasind)
#else
              CALL fill_cols_vals_rowptr_loc2glob(Neq, Nfp, Nf, Fi, blk, ifa, Fe, Fd, indpos, indglo, Kel, fel, rows, rhsvec, loc2glob, cols, vals, shift, linew)
#endif
              !!$OMP END CRITICAL

              Fe_aux = Fe
              CALL exchange_Fi(Fe, Fi, Fi_per, Nf) ! Swap Fi <-> Fi_per
              CALL getindposextperiodic(indpos, Fi_per, Fi, Nf, Nfp, neq, Fd, Fe, ieln)
...
CONTAINS
  ...
  SUBROUTINE exchange_Fi(Fe, Fi, Fi_per, Nf)
    INTEGER, INTENT(OUT)    :: Fi, Fi_per
    INTEGER, INTENT(IN)     :: Nf
    INTEGER, INTENT(INOUT)  :: Fe(1:Nf)
    INTEGER                 :: aux, i
  ...
  SUBROUTINE computeElementalMatrix(Kel, fel, iel)
   USE LinearAlgebra, ONLY: matmul_dgemm
    INTEGER, INTENT(IN)             :: iel
    REAL*8, INTENT(INOUT)           :: Kel(:,:), fel(:)
    REAL*8, POINTER                 :: A_lq(:,:), A_lu(:,:), A_ll(:,:)
    REAL*8, POINTER                 :: LL(:,:), L0(:), UU(:,:), U0(:), f(:)
    ...
    ! Elemental matrix and elemental RHS
    Kel = matmul_dgemm(A_lq, LL) + matmul_dgemm(A_lu, UU) + A_ll
    fel = -(RESHAPE(matmul_dgemm(A_lu, RESHAPE(U0, (/SIZE(U0), 1/))), SHAPE(fel)) + RESHAPE(matmul_dgemm(A_lq, RESHAPE(L0, (/SIZE(L0), 1/))), SHAPE(fel))) + f
  ...
  ! for interior faces
  !**************************************************
  SUBROUTINE getindposint(indpos, Fd, Fe, Neq, Nfp, Nf, iel, Fi)
    INTEGER, INTENT(IN)  :: Neq, Nfp, Nf, iel, Fi, Fe(Nf)
    LOGICAL, INTENT(IN)  :: Fd(Nf)
    INTEGER, INTENT(OUT) :: indpos(1:neq*Nfp*Nf)
    INTEGER              :: i, j, Fei
    INTEGER              :: pos(1:Nf)
    ...
    DO i = 1, Nf
       ...
       Fei = Fe(i)
       DO j = 1, Nf
          IF (i .EQ. j .OR. Fd(j)) CYCLE
          IF (Fei .GT. Fe(j)) THEN
             pos(i) = pos(i) + 1
          END IF
       END DO
       DO j = 1, Nf
          IF (Fen(j) .EQ. Fi .OR. Fdn(j)) CYCLE
          IF (Fei .GT. Fen(j)) THEN
             pos(i) = pos(i) + 1
          END IF
       END DO
    END DO
  ...
  ! for exterior faces
  !**************************************************
  SUBROUTINE getindposext(indpos, Fe, Fd, Nf, Neq, Nfp)
    INTEGER, INTENT(IN)    :: Nf, Neq, Nfp
    INTEGER, INTENT(OUT)   :: indpos(1:Neq*Nfp*Nf)
    INTEGER, INTENT(IN)    :: Fe(Nf)
    LOGICAL, INTENT(IN)    :: Fd(Nf)
  ...
  !**************************************************
  SUBROUTINE getindposextperiodic(indpos, Fi_per, Fi, Nf, Nfp, neq, Fd, Fe, ieln)
    
    INTEGER, INTENT(IN)  :: ieln, Fi, Fi_per, neq, Nfp, Nf, Fe(Nf)
    LOGICAL, INTENT(IN)  :: Fd(Nf)
    INTEGER, INTENT(OUT) :: indpos(1:neq*Nfp*Nf)
    INTEGER              :: i, j, fei
    INTEGER              :: pos(1:Nf)
  ...
  !**************************************************
#ifdef PARALL
  SUBROUTINE fill_cols_vals_rowptr_loc2glob(Neq, Nfp, Nf, Fi, blk, ifa, Fe, Fd, indpos, indglo, Kel, fel, rows, rhsvec, loc2glob, cols, vals, shift, linew, Fasind)
#else
  SUBROUTINE fill_cols_vals_rowptr_loc2glob(Neq, Nfp, Nf, Fi, blk, ifa, Fe, Fd, indpos, indglo, Kel, fel, rows, rhsvec, loc2glob, cols, vals, shift, linew)
#endif

    INTEGER, INTENT(IN)    :: Neq, Nfp, Nf, Fi, blk, ifa
    INTEGER, INTENT(IN)    :: Fe(Nf), indpos(Nf * Nfp * Neq)
    INTEGER, INTENT(IN)    :: shift(:), linew(:)
    INTEGER, INTENT(INOUT) :: indglo(Nf * Nfp * Neq), rows(:), loc2glob(:), cols(:)
    REAL*8, INTENT(IN)     :: Kel(blk * Nf, blk * Nf), fel(:)
    REAL*8, INTENT(INOUT)  :: rhsvec(:), vals(:)
    LOGICAL, INTENT(IN)    :: Fd(Nf)
#ifdef PARALL
    INTEGER, INTENT(IN)    :: Fasind(:)
#endif
    INTEGER                :: sl, i, j, k, jj, ifl
  ...
  SUBROUTINE init_mat()
    ...
    ALLOCATE(MatK%vals(nnz), rhs%vals(nn), SOURCE=0.0)
    ALLOCATE(MatK%rowptr(nn + 1), MatK%loc2glob(nn), rhs%loc2glob(nn), MatK%cols(nnz), SOURCE=0)
