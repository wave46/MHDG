...
CONTAINS
  ...
  SUBROUTINE create_reference_element(refEl, ndim, degree, verbose, ngauss_1d)
    TYPE(Reference_element_type), INTENT(OUT) :: refEl
    INTEGER, INTENT(in) :: ndim, verbose
    ...
    IF (PRESENT(degree)) THEN
       IF (refEl%elemType == 0) THEN
          refEl%Nnodes2D = INT(0.5*(degree + 1)*(degree + 2))
...
100    p1 = 1.d0
  ...
  SUBROUTINE generate_fekete_nodes(X,T,nDegRef,RefEl)
    ...
    REAL*8, DIMENSION(SIZE(X,1),SIZE(X,2))  :: X_mod
    INTEGER, DIMENSION(:,:), INTENT(IN)      :: T
    INTEGER, INTENT(IN)                      :: nDegRef
    INTEGER                                  :: elemType
    REAL*8                                  :: nodes(SIZE(T,2),2)
    REAL*8, DIMENSION(:,:)                  :: Vand(SIZE(T,2), SIZE(T,2)), Vand_trans(SIZE(T,2), SIZE(T,2)), invV(SIZE(T,2), SIZE(T,2))
    REAL*8                                  :: sf(SIZE(T,2))
    INTEGER                                  :: npoints, nelements
    INTEGER                                  :: i, j, k
    REAL*8                                  :: h
    REAL*8                                  :: p(SIZE(T,2)), shapeFunctions(SIZE(T,2), SIZE(T,2)), invSf(SIZE(T,2), SIZE(T,2))
    ...
    k = 4  ! The current row index

    ! Generate nodes for the remaining points
    DO i = 1, nDegRef - 1
       nodes(k, 1) = i * h
       nodes(k, 2) = 0.0d0
       k = k + 1
    ...
    IF(MOD(npoints,(nDegRef+1)*(nDegRef+2)/2) .EQ. 0) THEN
       elemType = 0
    ELSEIF(MOD(npoints,(nDegRef)*(nDegRef+1)) .EQ. 0) THEN
       elemType = 1
    ELSE
       WRITE(*,*) "THIS ELEM TYPE IS NOT CODED, look into reference_element"
       STOP
    ENDIF

    ! Apply the final transformation
    nodes = 2.0d0 * nodes - 1.0d0

    ! Check refElPol%elemType and call the appropriate functions
    IF (elemType == 0) THEN
       ! Triangles
       CALL vandermonde_2d(Vand, refEl)
    ELSEIF (elemType == 1) THEN
       ! Quadrilaterals
       CALL vandermonde_qua(Vand, refEl)
    ELSE
       WRITE (6, *) "Vandermonde matrix for this element type not coded yet"
       STOP
    END IF

    Vand_trans = TRANSPOSE(Vand)
    CALL invert_matrix(Vand_trans,invV)

    ! Compute shape functions at interpolation points

    DO i = 1, npoints
       CALL orthopoly2d(nodes(i, 1), nodes(i, 2), npoints, nDegRef, p)
       !CALL solve_linear_system_sing(Vand_trans, p, sf).
       sf = MATMUL(invV,p)
       shapeFunctions(i, :) = sf
    END DO

    CALL invert_matrix(shapeFunctions,invSf)
    DO i = 1, nelements
       X_mod(T(i,:),1) = MATMUL(invSf,X(T(i,:),1))
       X_mod(T(i,:),2) = MATMUL(invSf,X(T(i,:),2))
       !CALL solve_linear_system_sing(shapeFunctions, X(T(i,:),1), sf)
       !X_mod(T(i,:),1) = sf
       !CALL solve_linear_system_sing(shapeFunctions, X(T(i,:),2), sf)
       !X_mod(T(i,:),2) = sf
    ENDDO
  ...
  SUBROUTINE compute_shape_functions_at_interp_points(n_deg1,n_deg2,elemType, shapeFunctions)
    ...
    TYPE(Reference_element_type)                :: refEl2
    INTEGER, INTENT(IN)                         :: n_deg1, n_deg2, elemType
    REAL*8, INTENT(OUT)                         :: shapeFunctions(:,:)
    REAL*8, ALLOCATABLE                         :: p(:), sf(:)
    REAL*8, ALLOCATABLE                         :: Vand(:,:), Vand_trans(:,:)
    INTEGER                                     :: n_points1, n_points2
    INTEGER                                     :: i

    CALL create_reference_element(refEl1,2,n_deg1, verbose = 0)
    CALL create_reference_element(refEl2,2,n_deg2, verbose = 0)
    n_points1 = refEl1%Nnodes2D
    n_points2 = refEl2%Nnodes2D

    ALLOCATE(Vand(n_points1,n_points1))
    ALLOCATE(Vand_trans(n_points1,n_points1))
    ALLOCATE(sf(n_points1))
    ALLOCATE(p(n_points1))


    IF(elemType .EQ. 0) THEN
       CALL vandermonde_2d(Vand, refEl1)
    ELSEIF(elemType .EQ. 1) THEN
       CALL vandermonde_qua(Vand, refEl1)
    ELSE
       WRITE (6, *) "Vandermonde matrix for this element type not coded yet"
       STOP
    ENDIF
    ...
    DO i = 1, n_points2
       CALL orthopoly2d(refEl2%coord2d(i, 1), refEl2%coord2d(i, 2), refEl1%Nnodes2D, n_deg1, p)
       CALL solve_linear_system_sing(Vand_trans, p, sf)
       shapeFunctions(i, :) = sf
    ENDDO

    DEALLOCATE(Vand)
    DEALLOCATE(Vand_trans)
    DEALLOCATE(sf)
    DEALLOCATE(p)

    CALL free_reference_element_pol(refEl1)
    CALL free_reference_element_pol(refEl2)

  END SUBROUTINE compute_shape_functions_at_interp_points
