...
CONTAINS
   ...
   SUBROUTINE initializeShockCapturing()
      INTEGER                   :: Ne, shock, ierr

      IF ((adapt%adaptivity .EQV. .TRUE.) .AND. (adapt%shockcp_adapt .EQ. 0)) THEN
         IF (MPIvar%glob_id .EQ. 0) THEN
            WRITE (*, *) "Error: requested adaptivity but adaptivity shock capturing (shockcp_adapt) is set to 0. Set to 1 or 2. stop"
            CALL MPI_Abort(MPI_COMM_WORLD, -1, ierr)
         END IF
      END IF

      IF ((adapt%adaptivity .EQV. .TRUE.) .AND. (switch%shockcp .NE. 0)) THEN
         IF (switch%shockcp .NE. adapt%shockcp_adapt) THEN
            IF (MPIvar%glob_id .EQ. 0) THEN
               WRITE (*, *) "WARNING: shock capturing is activated both in switch and adapt. The highest values will be used (which is ", MAX(switch%shockcp, adapt%shockcp_adapt), ")."
            END IF
         END IF
      END IF

      IF (adapt%adaptivity .EQV. .FALSE.) THEN
      ...
      shock = MAX(switch%shockcp, adapt%shockcp_adapt)
      ...
      SELECT CASE (shock)
       ...
       CASE (3)
         ...
         ELSEIF (refElPol%Nvertices .EQ. 4) THEN
            ...
            refElPol%Nlin(:, 4) = 0.25*((1 - refElPol%coord2d(:, 1))*(1 + refElPol%coord2d(:, 2)))
         ELSE
            WRITE (6, *) "Not coded yet. stop"
            CALL MPI_Abort(MPI_COMM_WORLD, -1, ierr)
         END IF

       CASE DEFAULT
         WRITE (6, *) "Error: shock capturing case not valid. stop"
         CALL MPI_Abort(MPI_COMM_WORLD, -1, ierr)
      END SELECT
