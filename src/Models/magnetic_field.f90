...
CONTAINS
   ...
   END SUBROUTINE load_magnetic_field_Jtor
...
!!! end of variables for computation derivatives of the flux
      ...
      REAL*8, ALLOCATABLE, DIMENSION(:) :: xq, yq, Br_nodes, Bz_nodes, Bt_nodes, flux_nodes
      REAL*8                            :: x, y
      ...
      ! Just here so that we get a nice output in the log
      ...
      ALLOCATE (xvec(jp))
      ALLOCATE (yvec(ip))
      ALLOCATE (xq(Mesh%Nnodes), yq(Mesh%Nnodes))
      ALLOCATE (Br_nodes(Mesh%Nnodes), Bz_nodes(Mesh%Nnodes), Bt_nodes(Mesh%Nnodes), flux_nodes(Mesh%Nnodes))
      xvec = r2D(1, :)
      yvec = z2D(:, 1)
      xq = Mesh%X(:, 2)
      yq = Mesh%X(:, 1)

      CALL spline2d_vec(ip, yvec, jp, xvec, Br2D, xq, yq, Br_nodes)
      CALL spline2d_vec(ip, yvec, jp, xvec, Bz2D, xq, yq, Bz_nodes)
      CALL spline2d_vec(ip, yvec, jp, xvec, Bphi2D, xq, yq, Bt_nodes)
      CALL spline2d_vec(ip, yvec, jp, xvec, flux2D, xq, yq, flux_nodes)
      ...
      END IF
      ! Free memory
      DEALLOCATE (Br2D, Bz2D, Bphi2D, xvec, yvec, xq, yq, Br_nodes, Bz_nodes, Bt_nodes, flux_nodes)
   ...
   SUBROUTINE initialize_puff()
      ...
      END IF
#endif
   END SUBROUTINE initialize_puff
