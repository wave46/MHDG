         ...
         DO j = 1, N1D  ! TODO: check if I need to invert i and j
            ...
            SELECT CASE (switch%testcase)
             ...
             CASE (5:6)
               ...
               up(ind, 1) = 1.
             CASE (50:59)
               IF (size(Mesh%T, 2)*N1D .EQ. size(up, 1)) THEN
                  ! case in which analytical_solution is called to initialise the initial solution
                  ind_fluxel = (j - 1)*Mesh%Nnodes + Mesh%T(iel, i)
                  fluxel = phys%magnetic_flux(ind_fluxel)
                  fluxel = (fluxel - phys%Flux2Dmin)/(phys%Flux2Dmax - phys%Flux2Dmin)
                  sigma = 0.4
                  up(ind, 1) = 1.*EXP(-fluxel**2/(2*sigma**2))
               ELSE
             ...
             CASE (65)
               ...
               END IF
             CASE (80:89)
               !Define an analytical solution with a Gaussian shape respect to the normalized flux surface
               ind_fluxel = (j - 1)*Mesh%Nnodes + Mesh%T(iel, i)
               fluxel = phys%magnetic_flux(ind_fluxel)
               fluxel = (fluxel - phys%Flux2Dmin)/(phys%Flux2Dmax - phys%Flux2Dmin)
               sigma = 0.3
