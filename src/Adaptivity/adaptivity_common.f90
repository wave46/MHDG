...
CONTAINS
  ...
  SUBROUTINE set_order_mesh(p)
    ...
    DO elem = 1, n_elements
       ...
       !IF(elem .eq. 35) WRITE(*,*) local_coord_logical

       ! read the faces infos
       face_info = elem_int_face(elem,:)

       !IF(elem .eq. 35) WRITE(*,*) face_info
       ...
       !IF(elem .eq. 35) WRITE(*,*) n_already_meshed_faces
  ...
  CONTAINS
    ...
    ! SUBROUTINE project_to_spline

    !   ! project to spline and find y1
    !   nodes_on_spline_proj(:,1) = nodes_on_spline(:,1)
    !   nodes_on_spline_proj(:,2) = splines(isp)%spline%value(nodes_on_spline(:,1))

    ! ENDSUBROUTINE project_to_spline
    ...
    ! SUBROUTINE compute_jacobian_elem(Xel, flag)
    ...
    !    flag = .FALSE.
    !
    !    IF(ANY(detJ .lt. tol)) flag = .TRUE.
    !
    !  ENDSUBROUTINE compute_jacobian_elem
  ...
  SUBROUTINE relative_difference_solution(u_prec, u, flag)
    USE physics, ONLY: cons2phys
    REAL*8, INTENT(IN)                  :: u_prec(:)
    REAL*8, INTENT(IN)                  :: u(:)
    LOGICAL, INTENT(OUT)                :: flag
    REAL*8                              :: up_prec(Mesh%Nnodesperelem*Mesh%Nelems*phys%neq/phys%Neq, phys%npv), up(Mesh%Nnodesperelem*Mesh%Nelems*phys%neq/phys%Neq, phys%npv), diff(Mesh%Nnodesperelem*Mesh%Nelems*phys%neq/phys%Neq, phys%npv)
    INTEGER                             :: i, j, size

    CALL cons2phys(TRANSPOSE(RESHAPE(u, (/phys%Neq, Mesh%Nelems*Mesh%Nnodesperelem/))), up)
    CALL cons2phys(TRANSPOSE(RESHAPE(u_prec, (/phys%Neq, Mesh%Nelems*Mesh%Nnodesperelem/))), up_prec)

    size = Mesh%Nelems*Mesh%Nnodesperelem

    diff = ABS((up_prec-up))
    DO i = 1,size
       diff(i,:) = diff(i,:)/(0.5*(NORM2(up_prec,1)+NORM2(up,1)))
    ENDDO

    WRITE(*,*) "Relative difference: ", MAXVAL(diff(:,10))
    diff = ABS((up_prec-up))
    WRITE(*,*) "Absolute difference: ", MAXVAL(diff(:,10))
  ...
  ENDSUBROUTINE relative_difference_solution
