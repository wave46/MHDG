...
MDL_NGAMMATITENEUTRALKEPS=NGammaTiTeNeutralkeps
...
ifeq ($(MDL),$(MDL_NGAMMA))
 ...
 #MACROS+= -DRHSBC
 ADDMOD+=hdg_LimitingTechniques.o
else ifeq ($(MDL),$(filter $(MDL), $(MDL_NGAMMATITENEUTRALK) $(MDL_NGAMMATITENEUTRALKEPS)))
 RMDL=NGammaTiTe
 MACROS+= -DNGAMMA 
 MACROS+= -DTEMPERATURE
 MACROS+= -DNEUTRAL
 ...
 MACROS+= -DTHERMALCX
 #MACROS+= -DCONSTANTNEUTRALDIFF
 # Indicates we are either using k or k-epsilon
 MACROS+= -DTURBULENCE
 #MACROS+= -DBOHMLIMIT
 #MACROS+= -DNEUTRALCONVECTION
 #MACROS+= -DPINCH

 ifeq ($(MDL),$(MDL_NGAMMATITENEUTRALK))
  MACROS+= -DKEQUATION
 #MACROS+= -DDKLINEARIZED
 else ifeq ($(MDL),$(MDL_NGAMMATITENEUTRALKEPS))
  MACROS+= -DKEPSILON
 endif
 ADDMOD+=hdg_LimitingTechniques.o
